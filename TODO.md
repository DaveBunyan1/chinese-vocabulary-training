...
- [x] **Definition Sanitization:** Run a cleaning script over vocabulary dictionary definitions to strip out parenthetical glosses, usage notes, and bound-form entries (e.g. reduce `他` from full dictionary note down to `he; him; his`).
- [x] **Flexible Definition Matching:** Update practice answer validation to parse multi-meaning target strings into single accepted terms so answers like `"I"` pass for `"I; me; my"`.
